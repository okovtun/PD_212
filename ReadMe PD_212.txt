...
TODO:
1. Обеспечить вызов методов дерева без необходимости передачи в него Корня;
2. Деструктор должен очищать дерево, т.е., удалять из него все элементы;
3. Tree tree = { 3, 5, 8, 13, 21 };
4. Написать метод int depth(), который возвращает глубину дерева;
5. Написать метод void erase(int Data), который удаляет заданное значение из дерева;
...
8. Измерить производительность всех этих методов;
